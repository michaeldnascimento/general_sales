...
$stringUser = "SELECT id_login, nome_usuario_login, usuario_login FROM usuario WHERE   nivel_login = 1 ";
//echo $stringSql;
$resultadoUser = mysqli_query($linkComMysql, $stringUser);
$qtdUser = mysqli_num_rows($resultadoUser);
$user = array();

while ($linha = mysqli_fetch_assoc($resultadoUser)){
$user[] = $linha;
}



$stringVendas = "SELECT motivo_cliente, statusVendas_venda_cliente, nomeUsuario FROM clientes WHERE nomeUsuario = {$user} motivo_cliente = 'VENDA - NOVO CLIENTE' ";
//echo $stringSql;
$resultadoVendas = mysqli_query($linkComMysql, $stringVendas);
$qtdVendas = mysqli_num_rows($resultadoVendas);
$vendas = array();
...
<header class="navbar navbar-inverse navbar-fixed-top bs-docs-nav" role="banner">
  <div class="container">
    <div class="navbar-header">
      ...
      <a href="tabulacao-geral.php" class="navbar-brand">General Sales</a>
    </div>
...
<!--**********************************LISTAGEM******************************************** -->
		    ...
		    <div class="table-responsive col-md-12">
		        ...
		        <table id="relatorio-data" class="table table-striped table-bordered" cellspacing="0" cellpadding="0">
		            <thead>
		                <tr>
		                 <th>ID</th>
		                 <th>VENDEDOR</th>
			...
			while ($relatorio = mysqli_fetch_assoc($resultado)) 
        {
          ...
          echo  '
				 	<tr>
		                    ...
		                    <td>'.$relatorio['id_login'].'</td>
                        <td>'.$relatorio['nome_usuario_login'].'</td>
                        <td>'.$relatorio['usuario_login'].'</td>
