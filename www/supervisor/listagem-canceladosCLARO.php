...
$stringSql = "
	SELECT id_cliente, nome_cliente, codigo_cliente, cidade_cliente, estado_cliente, nomeUsuario, nomeEquipe, date_format(data_venda,'%d/%m/%Y')as data_venda, nomeUsuarioBack, statusPedido_venda_cliente, statusVenda_venda_cliente, date_format(data_inst_venda_cliente,'%d/%m/%Y')as data_inst_venda_cliente, date_format(data_canc_venda_cliente,'%d/%m/%Y')as data_canc_venda_cliente, date_format(data_agendamento_venda_cliente,'%d/%m/%Y')as data_agendamento_venda_cliente, date_format(data_canc_venda_cliente,'%d/%m/%Y')as data_canc_venda_cliente, motivo_cliente, numPacote_venda_cliente, tv_venda_cliente, internet_venda_cliente, netfone_venda_cliente, netcelular_venda_cliente, agregado_venda_cliente, formaPagemento_cliente, auditadoBack_venda_cliente, lista_sistema, origemCSV, status_mailing, flag FROM clientes WHERE lista_sistema = 'CANCELADOSCLARO' AND (status_mailing IS NULL OR status_mailing = 'ATIVO') ORDER BY id_cliente DESC 
	";
...
<div id="main" class="container-fluid">
    ...
    <div class="col-md-12">
        <a href="#" onclick="window.open('listagem-canceladosCLARO-inativos.php', 'Pagina', 'STATUS=NO, TOOLBAR=NO, LOCATION=NO, DIRECTORIES=NO, RESISABLE=NO, SCROLLBARS=YES, TOP=100, LEFT=200, WIDTH=400, HEIGHT=400');" class="btn btn-danger active btn-sm pull-left h2">INATIVOS</a>
          ...
          <div class="form-group">
            ...
            <input type="text" style="display: none" name="lista_csv" class="form-control input-md" id="lista_csv" value="<?=$stringSql?>">
             ...
             <button type="button" id="botao" class="btn btn-default active btn-xs pull-right" title="Tabulacao" onClick="ativa()"><span class="glyphicon glyphicon-list-alt"></span></button>
             </div>
          </div>
</div>
</div> 
...
<form action="listagem-canceladosCLARO.php" method="post">
    <div class="form-group">
...
<h2><center><strong>CANCELADOS CLARO</strong></center></h2>
...
<!--**********************************LISTAGEM******************************************** -->
<form action="status-listagem.php?lista=CANCELADOSCLARO" method="POST">
...
<!--*****************QUANTIDA DE COSTATOS DA PAGINA*************************** -->
		    	...
		    	<div class="form-group">
					<label class="col-md-9 control-label" for="button1id"></label>
						<div class="col-md-2">
						 <input type="submit" class="btn btn-warning"  name="submit" value="Inativar Selecionados"/>
