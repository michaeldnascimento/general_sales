...
$stringSql = "
	SELECT id_cliente, nome_cliente, codigo_cliente, cidade_cliente, estado_cliente, nomeUsuario, nomeEquipe, date_format(data_venda,'%d/%m/%Y')as data_venda, nomeUsuarioBack, statusVenda_venda_cliente, date_format(data_inst_venda_cliente,'%d/%m/%Y')as data_inst_venda_cliente, motivo_cliente, numPacote_venda_cliente, tv_venda_cliente, internet_venda_cliente, netfone_venda_cliente, netcelular_venda_cliente, agregado_venda_cliente, formaPagemento_cliente, auditadoBack_venda_cliente, lista_sistema, chipAtivo_cliente, tabulacao_chip FROM clientes WHERE (motivo_cliente = 'VENDA - BASE TV') {$sqlequipe} {$sqlempresa} AND (MONTH(data_venda)) = MONTH(NOW()) AND (MONTH(data_venda)) = MONTH(NOW()) AND (YEAR(data_venda)) = YEAR(NOW()) ORDER BY data_venda desc
	";
...
<div class="container">
         ...
         <!-- Text input-->
<form action="listavenda-backoffice-tv.php" method="post">
...
<br>
          ...
          <button type="submit" class="btn btn-success" data-toggle="modal" data-target="#myModal">Pesquisar</button>
		 	    ...
		 	    <div class="page-header">
  				<h2>Backoffice Vendas Base TV</h2>
				  </div>
